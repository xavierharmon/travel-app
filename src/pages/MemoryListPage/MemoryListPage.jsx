// src/pages/MemoryListPage/MemoryListPage.jsx
...
export default function MemoryListPage({ onNewMemory, onEditMemory, onBack }) {
  const { memories, loading, error, deleteMemory } = useMemories();
  const [sortBy, setSortBy] = useState("date-newest");

  // ← was named "sortedTrips", causing a ReferenceError in the render below
        ...
        <div className={styles.brand}>
          ...
          <div>
            ...
            <p className={styles.subtitle}>
              {loading
                ? "Loading…"
                : `${memories.length} memor${memories.length !== 1 ? "ies" : "y"} saved`}
            </p>
          </div>
        </div>

        <div style={{ display: "flex", gap: "var(--space-sm)", flexWrap: "wrap" }}>
          <Button variant="ghost" onClick={onBack} size="md">← Back</Button>
          <Button onClick={onNewMemory} size="md">+ New Memory</Button>
        </div>
      </header>
      ...
      <main className={styles.main}>
        {loading ? (
          ...
          <div className={styles.empty}>
            ...
            />
            <p>
              No memories yet. Click <strong>New Memory</strong> to get started!
            </p>
          </div>
        ) : (
